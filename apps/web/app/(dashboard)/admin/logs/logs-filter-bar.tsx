...
export function LogsFilterBar({ actions }: { actions: string[] }) {
  ...
  const fieldClass = "rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300 outline-none transition focus:border-primary/60";
  return <div className="relative grid gap-3 rounded-2xl border border-white/[.07] bg-zinc-950/60 p-5 sm:grid-cols-2 lg:grid-cols-3" aria-busy={isPending}>
    ...
    {isPending && <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-zinc-950/35 backdrop-blur-[1px]"><span className="flex items-center gap-2 rounded-full border border-primary/20 bg-zinc-950 px-3 py-1.5 text-xs font-semibold text-primary"><LoaderCircle size={14} className="animate-spin"/> Actualizando análisis</span></div>}
  </div>;
}
