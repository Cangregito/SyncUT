...
export default async function AdminLogsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="mx-auto flex max-w-[1500px] flex-col gap-6 pb-12">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.5fr_.7fr]">
      <form className="grid gap-3 rounded-2xl border border-white/[.07] bg-zinc-950/60 p-5 sm:grid-cols-2 lg:grid-cols-3"><input name="q" defaultValue={params.q} placeholder="Actor, acción, registro o motivo…" className="rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-white"/><select name="action" defaultValue={params.action} className="rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300"><option value="">Todas las acciones</option>{actions.map(action=><option key={action}>{action}</option>)}</select><select name="result" defaultValue={params.result} className="rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300"><option value="">Todos los resultados</option><option value="success">Exitosos</option><option value="failed">Fallidos</option><option value="denied">Denegados</option></select><select name="severity" defaultValue={params.severity} className="rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300"><option value="">Todas las severidades</option><option value="info">Informativa</option><option value="warning">Advertencia</option><option value="critical">Crítica</option></select><input name="from" type="date" defaultValue={params.from} className="rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300"/><div className="flex gap-2"><input name="to" type="date" defaultValue={params.to} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-zinc-950 px-3 py-2.5 text-sm text-zinc-300"/><button className="rounded-xl bg-violet-500 px-4 text-sm font-bold text-white">Analizar</button></div></form>
