...
export default async function AdminLogsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="mx-auto flex max-w-[1500px] flex-col gap-6 pb-12">
    <section className="rounded-2xl border border-primary/15 admin-hero p-6 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-primary">Centro de observabilidad</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-white">Auditoría y análisis de eventos</h1><p className="mt-3 max-w-3xl text-sm text-zinc-400">Evidencia trazable para seguridad, cumplimiento y análisis operativo. Los registros son inmutables.</p></div><a href={`data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`} download={`syncut-auditoria-${new Date().toISOString().slice(0,10)}.csv`} className="flex items-center gap-2 rounded-xl bg-primary-container px-4 py-3 text-sm font-bold text-on-primary-container"><Download size={17}/> Exportar {filtered.length} eventos</a></div>
    </section>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      ["Eventos analizados", filtered.length, Activity, "text-primary bg-primary/10"],
