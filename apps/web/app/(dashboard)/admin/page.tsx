...
export default async function AdminRoutePage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; q?: string; role?: string }> }) {
  ...
  return <main className="mx-auto flex max-w-[1500px] flex-col gap-6 pb-12">
    ...
    <section className="relative overflow-hidden rounded-2xl border border-violet-400/15 admin-hero p-6 md:p-8">
    ...
    <section className="grid gap-6 xl:grid-cols-[.85fr_1.65fr]">
      ...
      <article className="overflow-hidden rounded-2xl border border-white/[.07] bg-zinc-950/60">
        <div className="border-b border-white/[.07] p-5 md:p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-bold text-white">Directorio de usuarios</h2><p className="mt-1 text-xs text-zinc-500">Administra roles y privilegios de {profiles.length} cuentas</p></div><form className="flex flex-wrap gap-2"><input name="q" defaultValue={params.q} placeholder="Buscar nombre o correo…" className="min-w-0 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-white outline-none focus:border-violet-400/60"/><select name="role" defaultValue={roleFilter} className="rounded-lg border border-white/10 bg-zinc-950 px-3 py-2 text-xs text-zinc-300"><option value="all">Todos</option>{USER_ROLES.map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select><button className="rounded-lg border border-white/10 px-3 text-xs font-semibold text-zinc-300">Filtrar</button></form></div></div>
