...
const ROLE_COLORS: Record<UserRole, string> = {
  ...
  tutor: "border-primary/20 bg-primary/10 text-primary",
...
export default async function AdminRoutePage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; q?: string; role?: string }> }) {
  ...
  return <main className="mx-auto flex max-w-[1500px] flex-col gap-6 pb-12">
    ...
    <section className="relative overflow-hidden rounded-2xl border border-primary/15 admin-hero p-6 md:p-8">
      <div className="relative flex flex-wrap items-end justify-between gap-6">
        <div><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-bold uppercase tracking-[.18em] text-primary"><ShieldCheck size={14}/> Centro de gobernanza</div>
          <h1 className="max-w-3xl text-3xl font-bold tracking-tight text-white md:text-4xl">Usuarios, roles y seguridad<br/><span className="text-primary">en un solo lugar.</span></h1>
    ...
    <section className="grid gap-6 xl:grid-cols-[.85fr_1.65fr]">
      <article className="rounded-2xl border border-white/[.07] bg-zinc-950/60 p-5 md:p-6">
        <div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><UserPlus size={20}/></div><div><h2 className="font-bold text-white">Agregar personal</h2><p className="text-xs text-zinc-500">Invitación segura por correo institucional</p></div></div>
        <form action={inviteStaffAccount} className="mt-6 space-y-4">
          {[['full_name','Nombre completo','Ej. Ana Martínez','text'],['email','Correo institucional','nombre_apellido@utcjedu.onmicrosoft.com','email'],['department','Área o departamento','Ej. Tecnologías de la Información','text']].map(([name,label,placeholder,type]) => <label key={name} className="block"><span className="mb-1.5 block text-xs font-semibold text-zinc-400">{label}</span><input name={name} type={type} placeholder={placeholder} required={name !== 'department'} className="w-full rounded-xl border border-white/10 bg-black/20 px-3.5 py-3 text-sm text-white outline-none transition focus:border-primary/60 focus:ring-2 focus:ring-primary/10"/>{name === 'email' ? <span className="mt-1.5 block text-[11px] text-zinc-500">Docente: @utcjedu.onmicrosoft.com o @utcj.edu.mx · Tutor: @utcj.edu.mx</span> : null}</label>)}
          <label className="block"><span className="mb-1.5 block text-xs font-semibold text-zinc-400">Rol inicial</span><select name="role" className="w-full rounded-xl border border-white/10 bg-zinc-950 px-3.5 py-3 text-sm text-white outline-none focus:border-primary/60"><option value="teacher">Docente</option><option value="tutor">Tutor</option></select></label>
          <button className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary-container px-4 py-3 text-sm font-bold text-on-primary-container transition hover:brightness-110">Enviar invitación <ArrowUpRight size={16}/></button>
        </form>
      </article>

      <article className="overflow-hidden rounded-2xl border border-white/[.07] bg-zinc-950/60">
        <div className="border-b border-white/[.07] p-5 md:p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-bold text-white">Directorio de usuarios</h2><p className="mt-1 text-xs text-zinc-500">Administra roles y privilegios de {profiles.length} cuentas</p></div><form className="flex flex-wrap gap-2"><input name="q" defaultValue={params.q} placeholder="Buscar nombre o correo…" className="min-w-0 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-white outline-none focus:border-primary/60"/><select name="role" defaultValue={roleFilter} className="rounded-lg border border-white/10 bg-zinc-950 px-3 py-2 text-xs text-zinc-300"><option value="all">Todos</option>{USER_ROLES.map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select><button className="rounded-lg border border-white/10 px-3 text-xs font-semibold text-zinc-300">Filtrar</button></form></div></div>
