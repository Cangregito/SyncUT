  ...
  return <div className="space-y-3">
    ...
    {tutorId ? <div className="rounded border border-outline-variant bg-surface p-3">
      ...
      <div className="mt-1 grid grid-cols-7 gap-1">{days.map((date) => {
        ...
        const unavailable = date < today || weekday === 0 || weekday === 6 || daySlots.length === 0 || allSlotsBusy;
        return <button key={value} type="button" disabled={unavailable || !sameMonth} onClick={() => { setSelectedDate(value); setSelectedSlot(""); }} title={allSlotsBusy ? "Todos los horarios estan ocupados" : unavailable ? "Sin disponibilidad" : "Disponible"} className={`aspect-square rounded text-xs ${selectedDate === value ? "bg-primary text-on-primary" : unavailable || !sameMonth ? "text-on-surface-variant opacity-25" : "bg-surface-container text-on-surface hover:bg-primary-container"}`}>{date.getDate()}</button>;
      })}</div>
      <p className="mt-3 text-[11px] text-on-surface-variant">Los fines de semana, dias sin disponibilidad y dias con todos sus horarios ocupados aparecen deshabilitados.</p>
    </div> : null}
    {selectedDate ? <label className="block text-xs font-medium text-on-surface-variant">Horario disponible
      <select value={selectedSlot} onChange={(event) => setSelectedSlot(event.target.value)} required className="mt-1 w-full rounded border border-outline-variant bg-surface px-3 py-2 text-sm text-on-surface"><option value="">Selecciona horario</option>{selectedDaySlots.map((slot) => {
        const slotBusy = busy.has(`${selectedDate}|${slot.startsAt}|${slot.endsAt}`);
        return <option key={`${slot.startsAt}-${slot.endsAt}`} value={`${slot.startsAt}|${slot.endsAt}`} disabled={slotBusy}>{slot.startsAt.slice(0,5)}-{slot.endsAt.slice(0,5)}{slotBusy ? " ocupado" : ""}</option>;
      })}</select>
    </label> : null}
  </div>;
}
