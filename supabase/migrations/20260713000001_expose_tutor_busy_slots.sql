DROP FUNCTION IF EXISTS public.get_assigned_tutor_busy_dates(integer);

CREATE OR REPLACE FUNCTION public.get_assigned_tutor_busy_dates(p_days_ahead integer DEFAULT 93)
RETURNS TABLE(tutor_id uuid, scheduled_date date, starts_at time, ends_at time)
LANGUAGE sql
SECURITY DEFINER
STABLE
SET search_path = ''
AS $$
  SELECT appointment.tutor_id, appointment.scheduled_date, appointment.starts_at, appointment.ends_at
