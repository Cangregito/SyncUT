...
AS $$
DECLARE
  caller_email text; caller_name text; caller_role text; normalized_code text;
  target_team public.tutor_teams%ROWTYPE; student_code_value text;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Usuario no autenticado.' USING ERRCODE = '42501'; END IF;

  SELECT p.email, p.full_name, p.role::text INTO caller_email, caller_name, caller_role
  FROM public.profiles AS p WHERE p.id = auth.uid();
  ...
  END IF;
  SELECT tt.* INTO target_team FROM public.tutor_teams AS tt
  WHERE tt.join_code = normalized_code AND tt.is_active = true;
  IF NOT FOUND THEN RAISE EXCEPTION 'Codigo de equipo no valido o inactivo.' USING ERRCODE = 'P0002'; END IF;

  SELECT s.student_code INTO student_code_value FROM public.students AS s WHERE s.id = auth.uid();
  student_code_value := COALESCE(
    student_code_value,
    upper(substring(caller_email from '^al([0-9]{8})@utcj\.edu\.mx$')),
    'STU-' || upper(left(replace(auth.uid()::text, '-', ''), 12))
  );
  INSERT INTO public.students AS s (id, student_code, cohort, career, enrollment_date)
  VALUES (auth.uid(), student_code_value, 'Sin cohorte', 'Sin carrera asignada', CURRENT_DATE)
  ON CONFLICT (id) DO UPDATE SET updated_at = now();

  UPDATE public.tutor_team_members AS m SET status = 'removed'
  WHERE m.student_id = auth.uid() AND m.status = 'active' AND m.team_id <> target_team.id;
  UPDATE public.tutorship_assignments AS a SET status = 'transferred'
  WHERE a.student_id = auth.uid() AND a.status = 'active' AND a.tutor_id <> target_team.tutor_id;

  INSERT INTO public.tutor_team_members AS m (team_id, student_id, status)
  VALUES (target_team.id, auth.uid(), 'active')
  ON CONFLICT (team_id, student_id) DO UPDATE SET status = 'active', joined_at = now();
  INSERT INTO public.tutorship_assignments AS a (tutor_id, student_id, status)
